...
func (s *sUser) SignIn(ctx context.Context, in model.UserSignInInput) (token string, err error) {
	...
	// 生成JWT token
	token, err = service.Jwt().GenerateToken(ctx, user.Id)
	if err != nil {
		return "", err
	}
...
// GetProfile retrieves and returns current user info.
func (u *sUser) GetProfile(ctx context.Context) *entity.User {
	...
	var user *entity.User
	err := dao.User.Ctx(ctx).Where(do.User{Id: ctxUser.Id}).Scan(&user)
	if err != nil {
		return nil
	}
	return user
...
func (s *sUser) SendEmailCode(ctx context.Context, emailAddr string) (err error) {
	...
	err = redisClient.Set(context.Background(), redisKey, code, 5*time.Minute).Err()
